...
import {
  PlusIcon, PencilIcon, TrashIcon, MagnifyingGlassIcon, DocumentTextIcon, ArrowRightCircleIcon,
} from '@heroicons/react/24/outline'
import ConfirmDialog from '@/components/ui/ConfirmDialog'
import FormListbox from '@/components/ui/FormListbox'
import { Skeleton } from '@/components/ui/Skeleton'
import { getQuotations, deleteQuotation } from '@/lib/accounting/quotations'
import { quotationStatusColor, quotationStatusLabel } from '@/lib/accounting/calc'
...
const statusFilterOptions: { value: string; label: string }[] = [
  ...
  { value: 'converted', label: 'แปลงเป็นใบแจ้งหนี้' },
]
...
export default function QuotationsPage() {
  ...
  const load = async () => {
    setLoading(true)
    try { setItems(await getQuotations()) }
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
        {loading ? (
          ...
          <div className="py-16 text-center">
            <DocumentTextIcon className="w-10 h-10 text-gray-300 mx-auto" />
            ...
            <table className="w-full text-sm">
              <thead className="bg-gray-50 text-xs text-gray-500 uppercase">
                <tr>
                  ...
                  <th className="px-5 py-3 text-left font-semibold">ใช้ได้ถึง</th>
              ...
              <tbody className="divide-y divide-gray-50">
                {filtered.map((q) => (
                  <tr key={q.id} className="hover:bg-gray-50">
                    <td className="px-5 py-3 font-mono text-gray-900">{q.docNumber}</td>
                    <td className="px-5 py-3">
                      <div className="font-medium text-gray-900">{q.customerSnapshot?.name ?? '—'}</div>
                      {q.customerSnapshot?.code && (
                        <div className="text-xs text-gray-400 font-mono">{q.customerSnapshot.code}</div>
                      )}
                    </td>
                    <td className="px-5 py-3 text-gray-700">{formatDate(q.issueDate)}</td>
                    <td className="px-5 py-3 text-gray-700">{formatDate(q.validUntil)}</td>
                    <td className="px-5 py-3 text-right font-semibold tabular-nums">{formatCurrency(q.grandTotal)}</td>
                    <td className="px-5 py-3 text-center">
                      <span className={`inline-block text-xs px-2.5 py-1 rounded-full font-medium ${quotationStatusColor(q.status as QuotationStatus)}`}>
                    ...
                    <td className="px-5 py-3 text-right">
                      <div className="flex items-center justify-end gap-1">
                        {q.status !== 'converted' && q.status !== 'rejected' && (
                          <Link
                            href={`/admin/accounting/invoices/new?fromQuotation=${q.id}`}
                        ...
                        {q.status === 'converted' && q.convertedToInvoiceId && (
                          <Link
                            href={`/admin/accounting/invoices/new?id=${q.convertedToInvoiceId}`}
                        ...
                        <Link
                          href={`/admin/accounting/quotations/new?id=${q.id}`}
      ...
      <ConfirmDialog
        isOpen={!!deleteTarget}
        title="ลบใบเสนอราคา"
        message={`ต้องการลบใบเสนอราคา ${deleteTarget?.docNumber} ใช่หรือไม่?`}
