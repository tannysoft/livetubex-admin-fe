...
import { fetchSharedPlan, liffPlanUrl, shareErrorMessage, type SharedPlan, type SharedPlanItem } from '@/lib/equipment/plan-share'
import { formatDate, formatDateTime } from '@/lib/utils'
import { auth } from '@/lib/firebase'
import { initLiff, isLiffLoggedIn, liffLogin, openExternal, signInFirebaseWithLiff } from '@/lib/line-liff'
import SharePrintView from './SharePrintView'
...
/**
 * via 'link' = /share/plan (ใส่รหัส · เปิดในแอป LINE → ส่งต่อไป LIFF · admin ที่ login อยู่ไม่ต้องใส่รหัส)
 * via 'liff' = /freelancer/plan (login LINE → freelancer ที่ลงทะเบียนแล้วดูได้เลย ไม่งั้นถอยไปใส่รหัส)
 */
function SharedPlanPage({ via }: { via: 'link' | 'liff' }) {
  ...
  const load = async (pw: string) => {
    if (!shareId || (!pw && !trusted)) return
    setLoading(true)
    setError('')
    try {
      const p = await fetchSharedPlan(shareId, pw)
      setPlan(p)
      if (pw) writeSaved(shareId, pw)
      document.title = p.title
    } catch (err) {
      if (pw) writeSaved(shareId, null)
  ...
  // เปิดได้โดยไม่ใช้รหัส (LINE/admin) หรือเคยใส่รหัสในแท็บนี้แล้ว → เปิดให้เลย (ระหว่างนั้นโชว์ skeleton ไม่ใช่ฟอร์มรหัส)
  const [booting, setBooting] = useState(true)
  useEffect(() => {
    let alive = true
    let redirecting = false
    const open = (p: SharedPlan, asTrusted: boolean, pw = '') => {
      if (!alive) return
      setPlan(p)
      setTrusted(asTrusted)
    ...
    const boot = async () => {
      ...
      if (via === 'liff') {
        try {
          if (await initLiff()) {
            if (!(await isLiffLoggedIn())) { redirecting = true; await liffLogin(); return }
            await signInFirebaseWithLiff()
            try { open(await fetchSharedPlan(shareId, ''), true); return } catch (err) {
              const code = (err as { code?: string }).code
              if (code === 'functions/permission-denied') {
                if (alive) setNotice('บัญชี LINE นี้ยังไม่ได้ลงทะเบียนเป็น Freelancer — ใส่รหัสผ่านที่ได้รับแทน')
              } else throw err
            }
          }
        } catch (err) {
          if (alive) setNotice(shareErrorMessage(err))
        }
      } else {
        ...
        if (auth.currentUser) {
          try { open(await fetchSharedPlan(shareId, ''), true); return } catch { /* ไม่ได้รับสิทธิ์ — ใช้รหัสตามปกติ */ }
        }
      }
      const saved = readSaved(shareId)
      if (!saved) return
      try { open(await fetchSharedPlan(shareId, saved), false, saved) } catch { writeSaved(shareId, null) } // รหัสเปลี่ยนแล้ว/ลิงก์ปิด → กลับไปถามรหัส
    }
      ...
      <div className="min-h-dvh flex flex-col items-center justify-center px-5 py-10">
        <div className="w-full max-w-sm">
          ...
          >
            ...
            />
            {notice && !error && <p className="text-sm text-amber-700 bg-amber-50 rounded-lg px-3 py-2">{notice}</p>}
  ...
  if (printing) {
    const inLineApp = typeof navigator !== 'undefined' && /\bLine\//i.test(navigator.userAgent)
    return (
      <SharePrintView
        plan={plan}
        shareId={shareId}
        ...
        onOpenExternal={() => void openExternal(`${window.location.origin}/share/plan?s=${encodeURIComponent(shareId)}&print=1`)}
      />
    )
  }
      ...
      <header className="bg-white border-b border-gray-100 px-4 pt-4 pb-3 lg:px-6 lg:py-2.5">
        <div className="flex items-start lg:items-center justify-between gap-3">
          ...
          <button
            ...
            className="shrink-0 flex items-center gap-1 px-3 h-9 rounded-full text-sm font-medium text-gray-600 border border-gray-200 active:bg-gray-100"
          >
            <ArrowDownTrayIcon className="w-4 h-4" /> PDF
          </button>
          <button
            onClick={() => void load(trusted ? '' : password)}
            disabled={loading}
            aria-label="โหลดใหม่"
