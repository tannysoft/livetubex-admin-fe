...
import type {
  Customer, DocumentItem, Job, Quotation, QuotationStatus,
} from '@/lib/types'
...
interface Props {
  ...
  onSubmit: (value: QuotationFormValue) => Promise<void>
  onCancel: () => void
  isLoading?: boolean
  /** ถ้า true จะแสดงปุ่ม "บันทึกเป็นแบบร่าง" และ "บันทึกและส่ง" แยกกัน */
  showSplitSave?: boolean
}
...
export default function QuotationForm({ defaultValues, onSubmit, onCancel, isLoading }: Props) {
  ...
  const [validUntil, setValidUntil] = useState<string>(defaultValues?.validUntil?.slice(0, 10) ?? addDays(todayYmd(), 30))
  ...
  const [status, setStatus] = useState<QuotationStatus>(defaultValues?.status ?? 'draft')
  const [error, setError] = useState<string>('')

  // load default vatRate from companySettings (ถ้าเป็น quotation ใหม่)
  useEffect(() => {
  ...
  // ถ้ามี defaultValues และมี customerSnapshot — load customer
  useEffect(() => {
    if (!defaultValues?.customerSnapshot) return
    // สร้าง customer-like object จาก snapshot (เพื่อแสดงใน select)
  ...
  const handleSubmit = async (overrideStatus?: QuotationStatus) => {
    setError('')
    if (!customer) { setError('กรุณาเลือกลูกค้า'); return }
    if (!issueDate) { setError('กรุณาระบุวันที่ออก'); return }
    if (!validUntil) { setError('กรุณาระบุวันที่ครบกำหนด'); return }
    ...
    <div className="space-y-6">
      ...
      <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-5">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          ...
          <div>
            <label className={labelCls}>ใช้ได้ถึงวันที่ *</label>
            <FormDatePicker value={validUntil} onChange={setValidUntil} buttonClassName={inputCls} minDate={issueDate || undefined} />
          </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
        <div className="lg:col-span-2 bg-white rounded-2xl border border-gray-100 shadow-sm p-5">
          ...
          <textarea
            ...
            rows={6}
            className={inputCls}
            placeholder="เงื่อนไขการชำระเงิน, การจัดส่ง, หมายเหตุอื่นๆ ที่จะปรากฏในเอกสาร..."
          />
