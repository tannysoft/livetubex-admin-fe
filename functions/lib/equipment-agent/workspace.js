...
function daysIn(r, max = 92) {
    const out = [];
    for (let d = r.start, i = 0; d <= r.end && i < max; d = nextDay(d), i++)
        out.push(d);
    return out;
}
const YMD = /^\d{4}-\d{2}-\d{2}$/;
const NO_PORT_CATEGORIES = ['cable', 'support', 'lens', 'lighting', 'power'];
const norm = (s) => s.toLowerCase().replace(/\s+/g, ' ').trim();
const tight = (s) => s.toLowerCase().replace(/[^a-z0-9ก-๙]+/g, '');
class Workspace {
    constructor(plan, equipmentList, usage) {
        this.plan = plan;
        this.usage = usage;
        this.newDiagramIds = new Set();
    ...
    // ── อ่านข้อมูล ────────────────────────────────────────────────────────────
    draftQty(equipmentId, exceptItemId) {
        return this.items
     ...
     */
    available(e, exceptItemId, range) {
        const pr = this.planRange();
        if (!pr)
            return Math.max(0, e.quantity - this.draftQty(e.id, exceptItemId));
    ...
    parseUseRange(useFrom, useTo) {
        if (!useFrom && !useTo)
            return {};
        const pr = this.planRange();
        if (!pr)
            return 'แผนยังไม่มีวันที่ — กำหนดวันใช้รายแถวไม่ได้';
        if ((useFrom && !YMD.test(useFrom)) || (useTo && !YMD.test(useTo)))
            return 'useFrom/useTo ต้องเป็น YYYY-MM-DD';
        const start = useFrom || pr.start;
        const end = useTo || pr.end;
        if (start < pr.start || end > pr.end || start > end)
            return `วันใช้ต้องอยู่ในวันงาน ${pr.start} ถึง ${pr.end}`;
        if (start === pr.start && end === pr.end)
            return {};
    ...
    inventoryOverview() {
        const rows = [];
        for (const cat of Object.keys(types_1.CATEGORY_LABEL)) {
            const list = [...this.equipment.values()].filter((e) => e.category === cat && e.status !== 'retired');
            if (list.length === 0)
                continue;
            const count = (o) => list.filter((e) => (e.ownership ?? 'owned') === o)
                .reduce((s, e) => s + this.available(e), 0);
            rows.push(`${cat} (${types_1.CATEGORY_LABEL[cat]}): ${list.length} รายการ — ว่างตอนนี้ ของบริษัท ${count('owned')} ชิ้น, พาร์ทเนอร์ ${count('partner')}, ของเช่า ${count('rental')}`);
        }
    ...
    searchInventory(args) {
        ...
        const list = [...this.equipment.values()]
            ...
            if (words.length === 0)
                return true;
            const text = [e.code, e.name, e.brand, e.model, e.category, types_1.CATEGORY_LABEL[e.category], e.rentalVendor, e.partnerName, e.storageLocation].filter(Boolean).join(' ');
        ...
        if (list.length === 0)
            return 'ไม่พบอุปกรณ์ที่ตรงเงื่อนไข — ลองคำค้นอื่น หรือถ้าไม่มีจริงใช้ add_external_items (เช่าเพิ่ม)';
    ...
    listPlan() {
        ...
        const diagramLines = this.diagrams.map((d) => {
            const nodes = d.nodes.map((n) => {
                const used = (side) => (n[side === 'in' ? 'inputs' : side === 'out' ? 'outputs' : 'ios'] ?? [])
    ...
    resolveItem(ref) {
        ...
        for (const d of this.diagrams) {
            ...
            if (n)
                return this.items.find((i) => i.id === n.planItemId);
        }
    ...
    swapPositions(a, b) {
        ...
        A.toLocation = locB;
        B.toLocation = locA;
        for (const c of kidsA)
            c.toLocation = locB;
        for (const c of kidsB)
            c.toLocation = locA;
        let subs = 0;
        for (const d of this.diagrams) {
    ...
    place3d(objects) {
        return objects.map((o) => {
            ...
            if (o.itemId && !item)
                return `✗ ${o.itemId}: ไม่มีในรายการ`;
            if (o.kind === 'desk' && /foh/i.test(o.label ?? ''))
                return '– FOH วางให้อัตโนมัติอยู่แล้ว ไม่ต้องวาง';
            const kind = o.kind ?? (item?.category === 'camera' ? 'camera' : 'generic');
            const label = o.label?.trim() || item?.name || kind;
            const p = { zone: o.zone, kind, label, ...(item ? { itemId: item.id } : {}), ...(o.note ? { note: o.note } : {}) };
    ...
    addItemsInner(items) {
        return items.map((req) => {
            ...
            if (req.attachTo && !parent)
                return `✗ ${e.name}: attachTo ${req.attachTo} ไม่มีในรายการ (ใช้ item id ของกล้องจาก add_items/list_plan)`;
            if (parent?.attachedTo)
                return `✗ ${e.name}: ${parent.name} ติดกับแถวอื่นอยู่แล้ว — attachTo ต้องเป็นแถวแม่ (กล้อง)`;
            if (e.status === 'retired')
                return `✗ ${e.name}: เลิกใช้แล้ว`;
            ...
            if (same) {
                same.quantity += qty;
                if (req.toLocation)
                    same.toLocation = req.toLocation;
                if (req.note)
                    same.note = [same.note, req.note].filter(Boolean).join(' · ');
                return `✓ ${e.name}: รวมเข้าแถวเดิม ${same.id} → x${same.quantity}`;
            }
            const item = {
                id: (0, types_1.newId)(), equipmentId: e.id, code: e.code, name: e.name, category: e.category,
    ...
    updateItemsInner(updates) {
        return updates.map((u) => {
            ...
            if (u.useFrom != null || u.useTo != null) {
                ...
                if (e && (u.quantity ?? it.quantity) > this.available(e, it.id, r))
                    return `✗ ${it.name}: ช่วงวันนั้นว่างแค่ ${this.available(e, it.id, r)} ชิ้น`;
                delete it.useFrom;
                delete it.useTo;
                Object.assign(it, use);
                if (it.origin && it.origin !== 'owned' && !it.expenseId && r)
                    it.rentalDays = daysIn(r).length;
            }
            if (u.quantity != null) {
                const qty = Math.max(1, Math.floor(u.quantity));
                if (it.expenseId && qty !== it.quantity)
                    return `✗ ${it.name}: ลงบัญชีแล้ว แก้จำนวนไม่ได้`;
                const e = it.equipmentId ? this.equipment.get(it.equipmentId) : undefined;
                const r = this.itemRange(it) ?? undefined;
                if (e && qty > this.available(e, it.id, r))
                    return `✗ ${it.name}: ว่างแค่ ${this.available(e, it.id, r)} ชิ้น`;
                it.quantity = qty;
            }
            if (u.fromLocation != null)
                it.fromLocation = u.fromLocation;
            const notes = [];
            if (u.toLocation != null && u.toLocation !== (it.toLocation ?? '')) {
                ...
                for (const c of kids)
                    c.toLocation = u.toLocation;
                if (kids.length)
                    notes.push(`ของในชุด ${kids.length} ชิ้นย้ายตาม`);
            ...
            if (u.attachTo != null) {
                ...
                if (u.attachTo === '') {
                    ...
                    if (!parent || parent.id === it.id)
                        return `✗ ${it.name}: attachTo ${u.attachTo} ไม่มีในรายการ`;
                    if (parent.attachedTo)
                        return `✗ ${it.name}: ${parent.name} ติดกับแถวอื่นอยู่แล้ว`;
    ...
    removeItems(ids) {
        return ids.map((id) => {
            ...
            if (!it)
                return `✗ ${id}: ไม่พบ`;
            if (it.expenseId)
                return `✗ ${it.name}: ลงบัญชีเป็นรายจ่ายแล้ว ลบไม่ได้`;
    ...
    addNodes(diagramId, nodes) {
        const d = this.diagram(diagramId);
        if (!d)
            return `✗ ไม่พบผัง ${diagramId} — create_diagram ก่อน`;
        return nodes.map((req) => {
            const item = req.itemId ? this.items.find((i) => i.id === req.itemId) : undefined;
            if (req.itemId && !item)
                return `✗ ${req.itemId}: ไม่มีในรายการอุปกรณ์ของแผน (add_items ก่อน)`;
            if (!item && !req.label)
                return '✗ กล่องอิสระ (ของสถานที่/ลูกค้า) ต้องมี label';
            ...
            const d0 = types_1.DEFAULT_PORTS[category];
    ...
    updateNode(diagramId, nodeId, patch) {
        ...
        if (!d || !n)
            return `✗ ไม่พบกล่อง ${nodeId}`;
        if (patch.label != null && patch.label.trim())
            n.label = patch.label.trim();
        if (patch.sub != null) {
            if (patch.sub.trim())
                n.sub = patch.sub.trim();
            else
                delete n.sub;
        }
        if (patch.note != null) {
            if (patch.note.trim())
                n.note = patch.note.trim();
            else
                delete n.note;
        }
    ...
    addPorts(diagramId, nodeId, side, names) {
        const d = this.diagram(diagramId);
        const n = d && this.findNode(d, nodeId);
        if (!d || !n)
            return `✗ ไม่พบกล่อง ${nodeId}`;
    ...
    removeNodes(diagramId, nodeIds) {
        const d = this.diagram(diagramId);
        if (!d)
            return `✗ ไม่พบผัง ${diagramId}`;
        const gone = new Set(nodeIds.map((id) => this.findNode(d, id)?.id).filter((x) => !!x));
        d.nodes = d.nodes.filter((n) => !gone.has(n.id));
        d.edges = d.edges.filter((e) => !gone.has(e.from.nodeId) && !gone.has(e.to.nodeId));
    ...
    resolvePort(n, name, role) {
        const order = role === 'from' ? ['out', 'io'] : ['in', 'io'];
        for (const side of order) {
            const list = this.ports(n, side);
            let idx = list.findIndex((p) => p === name);
            if (idx < 0)
                idx = list.findIndex((p) => norm(p) === norm(name));
            if (idx < 0)
                idx = list.findIndex((p) => tight(p) === tight(name));
            if (idx >= 0)
    ...
    connect(diagramId, links) {
        ...
        return links.map((l) => {
            ...
            if (!a)
                return `✗ ไม่พบกล่อง ${l.fromNode}`;
            if (!b)
                return `✗ ไม่พบกล่อง ${l.toNode}`;
            if (a.id === b.id)
                return `✗ โยงเข้าตัวเองไม่ได้ (${a.label})`;
            ...
            if (this.portUsed(d, a.id, from.side, from.index))
                return `✗ ${a.label}.${l.fromPort} มีสายเสียบอยู่แล้ว — ใช้ port อื่น หรือผ่าน DA/router`;
            if (this.portUsed(d, b.id, to.side, to.index))
                return `✗ ${b.label}.${l.toPort} มีสายเสียบอยู่แล้ว`;
            const edge = {
                id: (0, types_1.newId)(),
    ...
    validate() {
        ...
        for (const id of byEq) {
            ...
            for (const day of pr ? daysIn(pr) : ['']) {
                ...
                if (need > free && (!worst || need - free > worst.need - worst.free))
                    worst = { day, need, free };
            }
            if (worst)
        ...
        for (const d of this.diagrams) {
            ...
            for (const [k, count] of seen) {
                ...
                issues.push({ level: 'error', message: `ผัง "${d.name}": ${n.label}.${this.ports(n, side)[Number(index)]} มีสาย ${count} เส้น (port ละ 1 เส้น)` });
            }
            for (const n of d.nodes) {
                if (NO_PORT_CATEGORIES.includes(n.category))
                    continue;
        ...
        if (noDest.length)
            issues.push({ level: 'warning', message: `ยังไม่ระบุปลายทาง ${noDest.length} รายการ: ${noDest.slice(0, 8).map((i) => i.name).join(', ')}${noDest.length > 8 ? '…' : ''}` });
        if (this.usage.noDate)
            issues.push({ level: 'warning', message: 'แผนยังไม่มีวันที่ — ยังเช็กของชนกับงานอื่นไม่ได้' });
        return issues;
    }
    validateText() {
