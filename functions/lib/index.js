...
const line_groups_1 = require("./line-groups");
admin.initializeApp();
...
const CORS_ORIGINS = [...ALLOWED_ORIGINS, /localhost/];
...
async function getBrandInfo() {
    ...
    try {
        ...
        const d = snap.data() ?? {};
        brandCache = {
            appName: d.appName || FALLBACK_BRAND.appName,
            primaryColor: /^#([0-9a-f]{3}|[0-9a-f]{6})$/i.test(d.primaryColor ?? '')
                ? d.primaryColor
...
async function getLineConfig() {
    ...
    try {
        ...
        liffId = (d.liffId ?? '').trim() || liffId;
        loginChannelId = (d.loginChannelId ?? '').trim() || loginChannelId;
    }
    catch (e) {
...
const SMTP_PASSWORD = (0, params_1.defineSecret)('SMTP_PASSWORD'); // รหัสผ่าน SMTP (ใช้เมื่อ provider = smtp)
const LINE_CHANNEL_ACCESS_TOKEN = (0, params_1.defineSecret)('LINE_CHANNEL_ACCESS_TOKEN'); // LINE Messaging API long-lived token
const LINE_CHANNEL_SECRET = (0, params_1.defineSecret)('LINE_CHANNEL_SECRET'); // ตรวจลายเซ็น webhook (lineWebhook)
// ── ชื่อย่อธนาคาร ────────────────────────────────────────────────────────────
const BANK_ABBR = {
...
exports.sendPaymentNotification = (0, https_1.onCall)({
    ...
    console.log(`[sendPaymentNotification] freelancer=${freelancerName} amount=${payment.amount} freelancerEmail=${freelancerEmail ?? 'none'} (freelancerId=${freelancerId ?? '-'} lineUserId=${lineUserId ?? '-'})`);
    ...
    const mail = await (0, mail_1.getMailSettings)();
    ...
    const from = (0, mail_1.buildFrom)(mail, creds, brand.appName);
    // ── เมลหา Admin ───────────────────────────────────────────────────────
    const adminTpl = mail.templates.paymentRequestAdmin;
    const adminTo = (0, mail_1.adminRecipients)(mail, creds);
    let adminEmailId;
    if (adminTpl.enabled && adminTo.length) {
        const html = (0, mail_1.renderEmailShell)({
            appName: brand.appName,
            primaryColor: brand.primaryColor,
            heading: (0, mail_1.renderVars)(adminTpl.heading, vars),
            intro: (0, mail_1.renderVars)(adminTpl.intro, vars),
            footer: (0, mail_1.renderVars)(adminTpl.footer, vars),
      ...
      <table style="width:100%;border-collapse:collapse;font-size:14px">
        ...
        const res = await (0, mail_1.sendMail)(mail, creds, {
            from,
            to: adminTo,
            subject: (0, mail_1.renderVars)(adminTpl.subject, vars),
            html,
        });
        if (res.error) {
            console.error('[sendPaymentNotification] ❌ ส่งเมลหา admin ไม่สำเร็จ:', res.error);
            throw new https_1.HttpsError('internal', `Email failed: ${res.error}`);
        }
        adminEmailId = res.id;
    ...
    if (freelancerEmail && flTpl.enabled) {
        const freelancerHtml = (0, mail_1.renderEmailShell)({
            appName: brand.appName,
            primaryColor: brand.primaryColor,
            heading: (0, mail_1.renderVars)(flTpl.heading, vars),
            intro: (0, mail_1.renderVars)(flTpl.intro, vars),
            footer: (0, mail_1.renderVars)(flTpl.footer, vars),
            bodyHtml: `
      <table style="width:100%;border-collapse:collapse;font-size:14px">
        ${row('รายละเอียดงาน', jobTitle)}
        ${row('วันที่ทำงาน', workDatesText)}
        ...
        const res = await (0, mail_1.sendMail)(mail, creds, {
            from,
            to: freelancerEmail,
            subject: (0, mail_1.renderVars)(flTpl.subject, vars),
            html: freelancerHtml,
        });
        if (res.error) {
            ...
            res.on('end', () => {
                try {
                    ...
                    if (res.statusCode === 200) {
                        ...
                        reject(new https_1.HttpsError('unauthenticated', `LINE token verification failed (${res.statusCode}): ${parsed.error_description ?? data}`));
                    }
                }
                catch {
                    reject(new https_1.HttpsError('internal', `Failed to parse LINE verify response: ${data}`));
                }
            });
        });
        req.on('error', (err) => {
            reject(new https_1.HttpsError('internal', `Network error calling LINE verify API: ${err.message}`));
        });
        req.setTimeout(10000, () => {
            req.destroy();
            reject(new https_1.HttpsError('deadline-exceeded', 'LINE verify API request timed out'));
        });
        req.end();
    });
}
...
exports.lineAuth = (0, https_1.onCall)({
    ...
    if (!loginChannelId) {
        ...
        throw new https_1.HttpsError('failed-precondition', 'ระบบยังตั้งค่า LINE Login channel ไม่เสร็จ กรุณาติดต่อ Admin');
    }
    let tokenInfo;
    try {
    ...
    if (tokenInfo.client_id !== loginChannelId) {
        console.warn(`[lineAuth] ปฏิเสธ token จาก channel อื่น: ได้ ${tokenInfo.client_id} คาดหวัง ${loginChannelId}`);
        throw new https_1.HttpsError('unauthenticated', 'LINE access token ไม่ได้ออกโดย channel ของระบบนี้');
    }
    if (tokenInfo.expires_in <= 0) {
        throw new https_1.HttpsError('unauthenticated', 'LINE access token หมดอายุแล้ว');
    }
    // ── 3. ดึง profile ────────────────────────────────────────────────────
    let lineProfile;
    try {
...
exports.sendPayoutNotification = (0, https_1.onCall)({
    ...
    const html = (0, mail_1.renderEmailShell)({
        ...
        maxWidth: 580,
        heading: (0, mail_1.renderVars)(tpl.heading, vars),
        intro: (0, mail_1.renderVars)(tpl.intro, vars),
        footer: (0, mail_1.renderVars)(tpl.footer, vars),
        bodyHtml: `
    ...
    if (freelancerEmail && tpl.enabled) {
        const res = await (0, mail_1.sendMail)(mail, creds, {
            from: (0, mail_1.buildFrom)(mail, creds, brand.appName),
            to: freelancerEmail,
            subject: (0, mail_1.renderVars)(tpl.subject, vars),
            html,
        });
        if (res.error) {
...
exports.lineWebhook = (0, https_1.onRequest)({ secrets: [LINE_CHANNEL_SECRET, LINE_CHANNEL_ACCESS_TOKEN], cors: false }, async (req, res) => {
    if (req.method !== 'POST') {
        res.status(405).send('Method Not Allowed');
        return;
    }
    const sig = req.get('x-line-signature');
    if (!(0, line_groups_1.verifyLineSignature)(req.rawBody, sig, LINE_CHANNEL_SECRET.value())) {
        res.status(401).send('Bad signature');
        return;
    }
    const events = Array.isArray(req.body?.events) ? req.body.events : [];
    try {
        await (0, line_groups_1.handleLineEvents)(events, LINE_CHANNEL_ACCESS_TOKEN.value());
    }
    catch (e) {
...
exports.sendPlanToLine = (0, https_1.onCall)({ cors: CORS_ORIGINS, secrets: [LINE_CHANNEL_ACCESS_TOKEN] }, async (request) => {
    if (!request.auth)
        throw new https_1.HttpsError('unauthenticated', 'Authentication required');
    const token = request.auth.token;
    if (token.firebase?.sign_in_provider !== 'password')
        throw new https_1.HttpsError('permission-denied', 'Admin only');
    const { planId, groupIds, freelancerIds, message } = (request.data ?? {});
    ...
    if (gIds.length + fIds.length === 0)
        throw new https_1.HttpsError('invalid-argument', 'เลือกกลุ่มหรือคนที่จะส่งก่อน');
    if (gIds.length > 20 || fIds.length > 100)
        throw new https_1.HttpsError('invalid-argument', 'ส่งได้ครั้งละไม่เกิน 20 กลุ่ม / 100 คน');
    const extra = typeof message === 'string' ? message.trim().slice(0, 1000) : '';
    const db = admin.firestore();
    const planSnap = await db.collection('equipmentPlans').doc(planId).get();
    if (!planSnap.exists)
        throw new https_1.HttpsError('not-found', 'ไม่พบแผน');
    const plan = planSnap.data();
    const share = (await db.collection('planShares').doc(planId).get()).data();
    if (!share?.enabled || !share.shareId)
        throw new https_1.HttpsError('failed-precondition', 'เปิดลิงก์แชร์ทีมงานของแผนนี้ก่อน (ปุ่ม “แชร์ทีมงาน”)');
    const brand = await getBrandInfo();
    const liffId = await getLiffId();
    const color = brand.primaryColor;
    const title = plan.title || 'แผนงาน';
    const s = encodeURIComponent(share.shareId);
    const planUri = (tab) => liffId ? `https://liff.line.me/${liffId}/plan?s=${s}&tab=${tab}` : `${APP_URL}/share/plan?s=${s}&tab=${tab}`;
    ...
    if (gIds.length) {
        ...
        for (const snap of snaps) {
            ...
            if (!g) {
                failed.push({ id: snap.id, name, reason: 'ไม่พบกลุ่ม' });
                continue;
            }
            if (g.active === false) {
                failed.push({ id: snap.id, name, reason: 'บอทออกจากกลุ่มแล้ว' });
                continue;
            }
            try {
...
exports.sendTestEmail = (0, https_1.onCall)({
    ...
    const { to, templateKey } = (request.data ?? {});
    const recipient = (to ?? '').trim();
    if (!/^[^@\s]+@[^@\s]+\.[^@\s]+$/.test(recipient)) {
        throw new https_1.HttpsError('invalid-argument', 'กรุณาระบุอีเมลผู้รับที่ถูกต้อง');
    }
    const brand = await getBrandInfo();
    const mail = await (0, mail_1.getMailSettings)();
    const creds = mailCreds({ withAdminTo: true });
    const key = (templateKey && templateKey in mail_1.DEFAULT_TEMPLATES)
        ? templateKey
        : 'paymentRequestAdmin';
    ...
    const res = await (0, mail_1.sendMail)(mail, creds, {
        from: (0, mail_1.buildFrom)(mail, creds, brand.appName),
        to: recipient,
        subject: `[ทดสอบ] ${(0, mail_1.renderVars)(tpl.subject, vars)}`,
        html,
    });
    if (res.error) {
        console.error('[sendTestEmail] ❌', res.error);
        throw new https_1.HttpsError('internal', res.error);
    }
...
exports.adminListUsers = (0, https_1.onCall)({ cors: ADMIN_CORS }, async (request) => {
    ...
    const users = list.users
        ...
        const role = doc?.role ?? (isBootstrapOwner(email) ? 'owner' : 'admin');
        return {
            uid: u.uid,
            email: u.email ?? '',
