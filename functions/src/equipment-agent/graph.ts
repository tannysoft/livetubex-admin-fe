...
/**
 * all = ทำทั้งหมดในรอบเดียว (คำสั่งแก้ร่างต่อ) · items = ขั้น 1 จัดของ · wiring = ขั้น 2 วาดผังระบบจากรายการที่จัดแล้ว
...
const PHASE_NOTES: Record<Exclude<AgentPhase, 'all'>, string> = {
  items: [
    ...
    'ยังไม่ต้องวาดผังระบบ (tool ผังปิดอยู่ในขั้นนี้) ขั้นถัดไประบบจะให้วาดผังจากรายการนี้เอง',
  ...
  wiring: [
    '[ขั้นที่ 2/2 — วาดผังระบบ]',
    'รายการอุปกรณ์จัดเสร็จแล้วในขั้นที่ 1 (อยู่ใน "ร่างปัจจุบัน") อย่าจัดใหม่หรือเอาของออก',
    'วาดผังระบบตามคำสั่งจากรายการนี้ เพิ่มของได้เฉพาะที่ขาดจริงสำหรับการโยงสาย (เช่น converter) และบอกใน summary',
    'รวบงานให้น้อยรอบ: add_nodes ทุกกล่องของผังในครั้งเดียว แล้ว connect ทุกเส้นของผังในครั้งเดียว',
    'ถ้าคำสั่งไม่ต้องการผังระบบเลย ให้ finish ทันที',
...
export async function runAgentGraph(opts: {
  ...
  // ขั้นจัดของ: ปิด tool ผังระบบ — โมเดลจะได้ไม่เผลอวาดผังจนเวลาหมดก่อนจัดของเสร็จ
      ...
      return {
        ...
        messages: [
          ...
          new HumanMessage(truncated
            ? 'คำตอบที่แล้วยาวเกินจนถูกตัดก่อนเรียก tool — อย่าวางแผนทั้งงานในหัวก่อน ลงมือเรียก tool ทีละส่วนเลย (เช่น add_items กล้อง → เลนส์ → converter/สวิตเชอร์ → ผังระบบ) แล้วจบด้วย finish'
